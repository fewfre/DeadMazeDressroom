package dressroom.data
{
	...
	public class Costumes
	{
		...
		public function init() : Costumes {
			...
			this.hair = _setupCostumeArray({ base:"M_1", type:ITEM.HAIR, pad:3, after:"_", map:tSkinParts });
			...
			this.shoes = _setupCostumeArray({ base:"M_55", type:ITEM.SHOES, pad:2, after:"_", map:tSkinParts });
			this.objects = _setupCostumeArray({ base:"dmo_", type:ITEM.OBJECT });
			...
			this.skins.push( new SkinData( 0, null ) );
			this.skins[this.skins.length-1].id = "Invisible";
			...
			for(var i = 0; i <= _MAX_COSTUMES_TO_CHECK_TO; i++) {
				if(pData.map) {
					for(var g:int = 0; g < (pData.sex ? 2 : 1); g++) {
						...
						if(tClassSuccess) {
							tArray.push( new ItemData({ id:i, type:pData.type, classMap:tClassMap, itemClass:tClassSuccess, gender:(pData.sex && tSexSpecificParts > 0 ? ((g==0?GENDER.FEMALE:GENDER.MALE)) : null) }) );
						}
		...
		public function getItemFromTypeID(pType:String, pID:int) : ItemData {
		...
		*****************************/
		...
		public function saveMovieClipAsBitmap(pObj:DisplayObject, pName:String="character", pScale:Number=1) : void
		{
			...
			var tBitmap:flash.display.BitmapData = new flash.display.BitmapData(pObj.width*pScale, pObj.height*pScale, true, 16777215);
