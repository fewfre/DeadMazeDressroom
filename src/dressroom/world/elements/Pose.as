package dressroom.world.elements
{
	...
	public class Pose extends MovieClip
	{
		...
		public function apply(pData:Object) : MovieClip {
			...
			for(var i:int = 0; i < _pose.numChildren; i++) {
				...
				for(var j:int = 0; j < tShopData.length; j++) {
					...
					if(part) {
						tItemsOnChild++;
						if(tShopData[j].type == ITEM.HAIR && pData.hairColor != null) Main.costumes.applyColorToObject(part, tHairColor);
		...
		private function _addToPoseIfCan(pSkinPart:DisplayObject, pData:ItemData, pID:String) : MovieClip {
			if(pData) {
